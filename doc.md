...
- SSE 连接每 20 秒发送注释心跳，降低连接被中间代理静默回收的概率。浏览器页面重新获得焦点、从后台恢复或从往返缓存恢复时，前端会重建 SSE 并重新读取服务端状态，补齐后台冻结期间错过的持久化消息和任务状态。
...
- DevStudio 不提供删除项目按钮，也不删除项目目录。用户可直接通过文件系统删除目录；后续尝试切换到该项目时，服务端会检测目录是否仍然存在，不存在则自动清理 `.devstudio/state.json` 中对应的项目、会话和消息记录，并通知前端刷新项目列表。
